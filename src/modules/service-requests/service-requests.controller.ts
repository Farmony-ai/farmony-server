...
export class ServiceRequestsController {
  ...
  async create(
    ...
    if (files && files.length > 0) {
      attachmentKeys = await Promise.all(
        files.map(file => this.s3Service.uploadFile(file, 'service-requests'))
      );
  ...
  async findAvailableForProvider(
    ...
    const providerRequests = availableRequests.requests.filter((request: any) =>
      request.allNotifiedProviders?.some(
        (pid: any) => pid.toString() === providerId
      )
    );
  ...
  async findOne(@Param('id') id: string, @Request() req) {
    ...
    const isNotifiedProvider = (request as any).allNotifiedProviders?.some(
  ...
  async decline(
    @Param('id') id: string,
    @Body() declineDto: DeclineServiceRequestDto,
    @Request() req,
  ) {
    const providerId = req.user.sub || req.user.userId;
  ...
  async getProviderStats(@Param('providerId') providerId: string) {
    ...
    const notifiedRequests = allRequests.requests.filter((request: any) =>
      request.allNotifiedProviders?.some(
        (pid: any) => pid.toString() === providerId
      )
    );
    
    const acceptedRequests = notifiedRequests.filter(
      (request: any) => request.acceptedProviderId?.toString() === providerId
    );
    
    const declinedRequests = notifiedRequests.filter((request: any) =>
      request.declinedProviders?.some(
        (pid: any) => pid.toString() === providerId
      )
    );
    ...
    return {
      ...
      totalPending: notifiedRequests.filter(
        (r: any) => r.status === 'MATCHED'
      ).length,
      acceptanceRate: notifiedRequests.length > 0
        ? (acceptedRequests.length / notifiedRequests.length) * 100
        : 0,
    };
  }
