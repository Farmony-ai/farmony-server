...
export class UsersController {
  ...
  async uploadProfilePicture(
    ...
    try {
      ...
      const s3Key = await this.s3Service.uploadFile(file, 'profile-pictures');

      // Update user with new profile picture S3 key
      const updatedUser = await this.usersService.updateProfilePicture(userId, s3Key);

      return {
        message: 'Profile picture uploaded successfully',
        profilePictureUrl: this.s3Service.getPublicUrl(s3Key),
        user: {
          id: updatedUser._id,
  ...
  async deleteProfilePicture(@Param('id') userId: string) {
    try {
      ...
      throw new BadRequestException(`Failed to delete profile picture: ${error.message}`);
    }
  }
}